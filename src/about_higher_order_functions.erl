...
fold_an_answer_from_the_left() ->
  "hello erlang koans" =:= lists:foldl(fun(Item, Accumulation) ->
                               string:join([Accumulation, Item], " ")
                             end, "hello", ["erlang", "koans"]).

or_from_the_right() ->
  "hello koans erlang" =:= lists:foldr(fun(Item, Accumulation) ->
                               string:join([Accumulation, Item], " ")
                             end, "hello", ["erlang", "koans"]).
...
function_passed_to_a_function() ->
  YourListReversalFunction = fun(List) -> lists:foldr(
    fun(Item, Accumulation) -> Accumulation ++ [Item] end, 
    [],
    List)
  end,
